...
// Проверяем, есть ли вообще что-то в корзине (в сессии)
if (!empty($_SESSION['basket'])) {
    // В сессии хранятся просто ID-шники товаров [2, 5, 9]. 
    // array_map('intval', ...) гарантирует, что все ID — это строго целые числа (защита)
    // implode(',', ...) склеивает их в строку "2,5,9", чтобы вставить в SQL-запрос WHERE id IN (...)
    $ids = implode(',', array_map('intval', $_SESSION['basket']));
    
    $stmt = $pdo->query("SELECT * FROM products WHERE id IN ($ids)");
    $basket_products = $stmt->fetchAll();
    
    // Считаем общую стоимость всех товаров, которые лежат в корзине
...
<body>
    ...
    <main>
        ...
        <div class="basket-layout">
            <div>
                <?php 
                // Если массив товаров пуст, пишем, что корзина пустая
            ...
            <?php 
            // Форму оформления заказа показываем ТОЛЬКО если в корзине есть хотя бы один товар
            if(!empty($basket_products)): 
            ?>
            <div>
